...
extern "C" {
    ...
    struct llama_model_params {
        ...
        bool use_external_tensors;  // SnapLLM: load structure only, skip tensor data (for vPID)
        bool vdpe_enable;           // SnapLLM vDPE: enable F32 cache save/load

        // SnapLLM vDPE: Path to directory for F32 tensor cache
        // If vdpe_enable=true and cache exists, load F32 from cache (fast path)
        // If vdpe_enable=true and cache doesn't exist, save F32 after dequantization
        const char * vdpe_cache_path;

        // SnapLLM: Pointer to VPIDWorkspace for memory-mapped tensor storage
        // If not null, vDPE will use workspace.bin instead of individual .f32 files
        // This enables HOT/WARM caching where tensors stay on disk and only hot pages load to RAM
        void * vpid_workspace;
    };
    ...
    // Used when model was loaded with use_external_tensors=true
    // Returns true on success, false if tensor not found or validation failed
    LLAMA_API bool llama_model_set_external_tensor(
