...
// SnapLLM vDPE: Tensor entry from workspace manifest
struct VDPETensorEntry {
    std::string name;
    size_t offset;
    size_t size;
...
// SnapLLM vDPE: Layer state for dynamic loading/eviction
struct LayerState {
    int layer_id;
    bool is_wired;
...
struct llama_model {
    ...
    // SnapLLM vDPE: F32 cache save/load functions
    bool vdpe_load_cache(llama_model_loader & ml);
    bool vdpe_save_cache(llama_model_loader & ml);
    bool vdpe_check_workspace();  // Check if VPIDWorkspace manifest exists
    bool vdpe_wire_workspace();   // Wire F32 data from VPIDWorkspace
    ...
    // SnapLLM vDPE: Dynamic layer loading/eviction
    void vdpe_init_layer_tracking();              // Initialize layer state tracking
    void vdpe_ensure_layer_loaded(int layer_id) const;  // Load layer on-demand if not wired
    void vdpe_load_layer(int layer_id) const;           // Load specific layer into RAM
    void vdpe_evict_layer(int layer_id) const;          // Evict layer from RAM
    void vdpe_wire_single_tensor(const std::string& name, const VDPETensorEntry& entry) const;
    void vdpe_unwire_single_tensor(const std::string& name) const;
