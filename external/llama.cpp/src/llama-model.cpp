...
#include "ggml-cpp.h"
#include "../ggml/src/ggml-quants.h"  // For dequantization functions
// SnapLLM: Include VPIDWorkspace for memory-mapped tensor storage
#include "../../../include/snapllm/vpid_workspace.h"
...
// SnapLLM vDPE: Platform-specific directory creation
#ifdef _WIN32
#include <direct.h>
#endif
...
// vDPE manifest functions - save/load tensor metadata for F32 cache
// Note: VDPETensorEntry is defined in llama-model.h
static bool vdpe_save_manifest(const std::string& manifest_path, const std::vector<VDPETensorEntry>& entries) {
    // Create directory if it doesn't exist
    std::string dir_path = manifest_path.substr(0, manifest_path.find_last_of("/\\"));
    ...
    if (!file.is_open()) {
        LLAMA_LOG_ERROR("%s: Failed to create manifest file: %s\n", __func__, manifest_path.c_str());
        return false;
    }

    // Write number of tensors
    uint64_t num_tensors = entries.size();
    file.write(reinterpret_cast<const char*>(&num_tensors), sizeof(num_tensors));

    // Write each tensor entry
    for (const auto& entry : entries) {
        // Write name length and name
        uint32_t name_len = static_cast<uint32_t>(entry.name.size());
        file.write(reinterpret_cast<const char*>(&name_len), sizeof(name_len));
        file.write(entry.name.data(), name_len);

        // Write offset and size
        file.write(reinterpret_cast<const char*>(&entry.offset), sizeof(entry.offset));
        file.write(reinterpret_cast<const char*>(&entry.size), sizeof(entry.size));

        // Write dimensions (4 int64_t values)
        file.write(reinterpret_cast<const char*>(entry.ne), sizeof(entry.ne));
    }

    file.close();
    LLAMA_LOG_INFO("%s: vDPE: Saved manifest with %zu entries to: %s\n", __func__, entries.size(), manifest_path.c_str());
    return true;
}

static bool vdpe_load_manifest(const std::string& manifest_path, std::vector<VDPETensorEntry>& entries) {
    ...
    for (uint64_t i = 0; i < num_tensors; i++) {
        ...
        // Read dimensions (4 int64_t values)
        file.read(reinterpret_cast<char*>(entry.ne), sizeof(entry.ne));
...
bool llama_model::vdpe_check_workspace() {
    if (params.vpid_workspace == nullptr) {
        return false;
    }

    auto* workspace = static_cast<snapllm::VPIDWorkspace*>(params.vpid_workspace);

    if (!workspace->is_initialized()) {
        return false;
    }

    // Check if workspace has data (file size > 1GB indicates existing F32 cache)
    size_t workspace_size = workspace->get_total_size();

    if (workspace_size < 1024ULL * 1024 * 1024) {  // Less than 1GB
        return false;  // Empty or invalid workspace
    }

    LLAMA_LOG_INFO("%s: vDPE: Found workspace with %.2f GB data\n",
                   __func__, (double)workspace_size / (1024.0 * 1024.0 * 1024.0));

    return true;
}

// SnapLLM vDPE: Wire F32 data from VPIDWorkspace using external tensor API
// This is called AFTER tensors_by_name is populated with structure
bool llama_model::vdpe_wire_workspace() {
    if (params.vpid_workspace == nullptr) {
        LLAMA_LOG_ERROR("%s: vDPE: No VPIDWorkspace available\\n", __func__);
        return false;
    }

    auto* workspace = static_cast<snapllm::VPIDWorkspace*>(params.vpid_workspace);

    // Load manifest to get correct offsets
    std::string manifest_path = std::string(params.vdpe_cache_path) + "/workspace_manifest.bin";
    std::vector<VDPETensorEntry> manifest_entries;

    if (!vdpe_load_manifest(manifest_path, manifest_entries)) {
        LLAMA_LOG_ERROR("%s: vDPE: Failed to load manifest from %s\\n", __func__, manifest_path.c_str());
        return false;
    }

    LLAMA_LOG_INFO("%s: vDPE: Loaded manifest with %zu tensors\\n", __func__, manifest_entries.size());
    LLAMA_LOG_INFO("%s: vDPE: Wiring %zu F32 tensors from workspace using manifest...\\n",
                   __func__, manifest_entries.size());
    ...
    for (const auto& entry : manifest_entries) {
        // Find tensor in tensors_by_name
        ggml_tensor* tensor = nullptr;
        for (const auto& pair : tensors_by_name) {
            if (pair.first == entry.name) {
                tensor = pair.second;
                break;
            }
        }
        ...
        if (!tensor) {
            LLAMA_LOG_WARN("%s: vDPE: Tensor '%s' from manifest not found in model\\n", __func__, entry.name.c_str());
            failed_count++;
            continue;
        }

        // Get direct mmap pointer at the offset from manifest
        const void* tensor_data = workspace->get_mmap_pointer(entry.offset);

        if (tensor_data == nullptr) {
            LLAMA_LOG_ERROR("%s: vDPE: Failed to get mmap pointer for tensor '%s' at offset %zu\\n",
                           __func__, entry.name.c_str(), entry.offset);
            failed_count++;
            continue;
        }

        // Wire the tensor with external data pointer
        if (llama_model_set_external_tensor(this, entry.name.c_str(), const_cast<void*>(tensor_data), GGML_TYPE_F32)) {
            wired_count++;
        } else {
            LLAMA_LOG_WARN("%s: vDPE: Failed to wire tensor '%s'\\n", __func__, entry.name.c_str());
            failed_count++;
        }
    }

    LLAMA_LOG_INFO("%s: vDPE: Wired %zu/%zu tensors (%zu failed)\\n",
                   __func__, wired_count, manifest_entries.size(), failed_count);

    // Success if we wired at least 90% of tensors
    return (wired_count >= manifest_entries.size() * 9 / 10);
}
// SnapLLM vDPE: Load tensors from F32 cache directory
bool llama_model::vdpe_load_cache(llama_model_loader & ml) {
    std::string cache_dir = params.vdpe_cache_path;

    // IMPORTANT: Clear tensors_by_name in case of previous failed load attempts
    tensors_by_name.clear();

    // Check if cache directory exists
    struct stat st;
    #ifdef _WIN32
        if (stat(cache_dir.c_str(), &st) != 0 || !(st.st_mode & _S_IFDIR)) {
            tensors_by_name.clear();  // Clean up before returning
            return false;  // Cache directory doesn't exist
        }
    #else
        if (stat(cache_dir.c_str(), &st) != 0 || !S_ISDIR(st.st_mode)) {
            tensors_by_name.clear();  // Clean up before returning
            return false;  // Cache directory doesn't exist
        }
    #endif

    // Check if cache manifest file exists
    std::string manifest_path = cache_dir + "/vdpe_cache.manifest";
    std::ifstream manifest(manifest_path, std::ios::binary);
    if (!manifest.is_open()) {
        return false;  // Manifest doesn't exist
    }

    // Read manifest: number of tensors
    uint64_t num_tensors = 0;
    manifest.read(reinterpret_cast<char*>(&num_tensors), sizeof(num_tensors));
    manifest.close();
    ...
    LLAMA_LOG_INFO("%s: vDPE: Loading %llu F32 tensors from cache...\n", __func__, (unsigned long long)num_tensors);

    try {
        // Calculate total memory needed for context
        size_t ctx_size = num_tensors * ggml_tensor_overhead();

        // Create ggml context for cached tensors
        struct ggml_init_params ggml_params = {
            /*.mem_size   =*/ ctx_size,
            /*.mem_buffer =*/ nullptr,
            /*.no_alloc   =*/ true,  // Don't allocate memory yet - we'll use backend buffer
        };
        ggml_context_ptr ctx_ptr { ggml_init(ggml_params) };
        if (!ctx_ptr) {
            LLAMA_LOG_ERROR("%s: vDPE: Failed to create ggml context\n", __func__);
            return false;
        }
        ggml_context * ctx = ctx_ptr.get();

        // Get backend buffer type for F32 tensors
        ggml_backend_buffer_type_t buft = ggml_backend_cpu_buffer_type();

        // Load tensor metadata from cache files
        size_t loaded_count = 0;

        // We need to iterate in the same order as save (which uses tensors_by_name from the previous load)
        // But tensors_by_name is empty now, so we iterate through ml.weights_map which provides tensor metadata
        for (const auto & [name_str, weight] : ml.weights_map) {
            std::string tensor_path = cache_dir + "/" + name_str + ".f32";
            std::ifstream tensor_file(tensor_path, std::ios::binary);

            if (!tensor_file.is_open()) {
                LLAMA_LOG_ERROR("%s: vDPE: Failed to open cache file: %s\n", __func__, name_str.c_str());
                tensors_by_name.clear();  // Clean up before returning
                return false;
            }
            ...
            tensor_file.read(reinterpret_cast<char*>(dims), sizeof(dims));

            // Read and verify tensor name
            uint32_t name_len;
            tensor_file.read(reinterpret_cast<char*>(&name_len), sizeof(name_len));
            std::string cached_name(name_len, '\0');
            tensor_file.read(&cached_name[0], name_len);

            tensor_file.close();  // Close for now, will reopen for data loading later

            if (cached_name != name_str) {
                LLAMA_LOG_ERROR("%s: vDPE: Name mismatch for %s (cached: %s)\n",
                               __func__, name_str.c_str(), cached_name.c_str());
                return false;
            }

            // Create tensor with cached dimensions (as F32)
            ggml_tensor * tensor = nullptr;
            if (dims[3] > 1) {
                tensor = ggml_new_tensor_4d(ctx, GGML_TYPE_F32, dims[0], dims[1], dims[2], dims[3]);
            } else if (dims[2] > 1) {
            ...
            if (!tensor) {
                LLAMA_LOG_ERROR("%s: vDPE: Failed to create tensor: %s\n", __func__, name_str.c_str());
                return false;
            }

            ggml_set_name(tensor, cached_name.c_str());

            // Add to tensors_by_name
            tensors_by_name.emplace_back(cached_name, tensor);

            loaded_count++;

            if (loaded_count % 50 == 0) {
                LLAMA_LOG_INFO("%s: vDPE: Loaded %zu/%llu tensors...\n", __func__, loaded_count, (unsigned long long)num_tensors);
            }
        }

        // Allocate all tensors in the backend buffer
        LLAMA_LOG_INFO("%s: vDPE: Allocating tensors in backend buffer...\n", __func__);
        ggml_backend_buffer_t allocated_buf = ggml_backend_alloc_ctx_tensors_from_buft(ctx, buft);
        if (!allocated_buf) {
            LLAMA_LOG_ERROR("%s: vDPE: Failed to allocate context tensors\n", __func__);
            return false;
        }

        // Now load F32 data into allocated tensors
        LLAMA_LOG_INFO("%s: vDPE: Loading F32 data into tensors...\n", __func__);
        size_t data_loaded_count = 0;
        for (const auto & [name, tensor] : tensors_by_name) {
            std::string tensor_path = cache_dir + "/" + name + ".f32";
            std::ifstream tensor_file(tensor_path, std::ios::binary);
            ...
            // Skip header (dims, name, size)
            int64_t dims[GGML_MAX_DIMS];
            tensor_file.read(reinterpret_cast<char*>(dims), sizeof(dims));

            uint32_t name_len;
            tensor_file.read(reinterpret_cast<char*>(&name_len), sizeof(name_len));
            tensor_file.seekg(name_len, std::ios::cur);  // Skip name

            uint64_t f32_size;
            tensor_file.read(reinterpret_cast<char*>(&f32_size), sizeof(f32_size));

            // Read F32 data
            std::vector<float> f32_data(f32_size / sizeof(float));
            tensor_file.read(reinterpret_cast<char*>(f32_data.data()), f32_size);
            tensor_file.close();

            // Copy data to tensor buffer
            ggml_backend_tensor_set(tensor, f32_data.data(), 0, f32_size);

            data_loaded_count++;
            if (data_loaded_count % 50 == 0) {
                LLAMA_LOG_INFO("%s: vDPE: Data loaded %zu/%zu tensors...\n", __func__, data_loaded_count, loaded_count);
            }
        }

        // Store context and buffer in model
        // Note: allocated_buf is managed by the context, we don't need to track it separately
        std::vector<ggml_backend_buffer_ptr> buffers;  // Empty - buffer is managed by allocator
        pimpl->ctxs_bufs.emplace_back(std::move(ctx_ptr), std::move(buffers));

        LLAMA_LOG_INFO("%s: ✅ vDPE: Loaded %zu/%llu tensors from cache (fast path!)\n",
                       __func__, data_loaded_count, (unsigned long long)num_tensors);

        return (data_loaded_count == num_tensors);

    } catch (const std::exception & e) {
        LLAMA_LOG_ERROR("%s: vDPE: Exception during cache loading: %s\n", __func__, e.what());
        return false;
    }
}

// SnapLLM vDPE: Save tensors to F32 cache directory
// SnapLLM vDPE: Save F32 tensors to VPIDWorkspace (NEW APPROACH)
bool llama_model::vdpe_save_cache(llama_model_loader & ml) {
    // UNCONDITIONAL skip - vDPE save disabled for all modes
    LLAMA_LOG_INFO("%s: vDPE: UNCONDITIONAL SKIP - save disabled\n", __func__);
    return true;

    // GPU mode early exit - don't save F32 cache (wastes 25GB disk writes)
    if (!params.use_external_tensors) {
        LLAMA_LOG_INFO("%s: vDPE: Skipping save - GPU mode (use_external_tensors=false)\n", __func__);
        return true;  // Return success to avoid error messages
    }

    // Check if VPIDWorkspace is available
    if (params.vpid_workspace == nullptr) {
        LLAMA_LOG_ERROR("%s: vDPE: No VPIDWorkspace available for saving\n", __func__);
        return false;
    }
    ...
    LLAMA_LOG_INFO("%s: vDPE: Saving %zu F32 tensors to VPIDWorkspace...\n", __func__, tensors_by_name.size());

    // Build manifest entries while saving
    std::vector<VDPETensorEntry> manifest_entries;
    manifest_entries.reserve(tensors_by_name.size());

    size_t current_offset = 0;
    size_t saved_count = 0;

    // Save each tensor to workspace
    for (const auto & [name, tensor] : tensors_by_name) {
        // Skip tensors with empty names
        if (name.empty()) {
            LLAMA_LOG_WARN("%s: vDPE: Skipping tensor with empty name\n", __func__);
            continue;
        }

        // Get actual tensor name from ggml
        const char * tensor_name = ggml_get_name(tensor);
        if (tensor_name == nullptr || strlen(tensor_name) == 0) {
            LLAMA_LOG_WARN("%s: vDPE: Skipping tensor - ggml_get_name returned empty\n", __func__);
            continue;
        }

        const int64_t nelements = ggml_nelements(tensor);
        const size_t f32_size = nelements * sizeof(float);

        // Debug: print first few tensor names to understand iteration order (use WARN so it always prints)
        static int iter_count = 0;
        if (iter_count < 5) {
            LLAMA_LOG_WARN("%s: vDPE ITER[%d]: '%s' buffer=%p type=%s\n",
                          __func__, iter_count, tensor_name, (void*)tensor->buffer, ggml_type_name(tensor->type));
            iter_count++;
        }

        // Validate tensor buffer before accessing
        if (tensor->buffer == nullptr) {
            LLAMA_LOG_WARN("%s: vDPE: Tensor '%s' has NULL buffer, skipping\n", __func__, tensor_name);
            continue;
        }

        // Allocate F32 buffer for dequantization
        std::vector<float> f32_data(nelements);

        // Dequantize tensor to F32 based on type
        bool data_retrieved = false;

        // Get tensor data - works for both host AND GPU buffers!
        // For GPU tensors, ggml_backend_tensor_get downloads data from GPU
        const size_t tensor_bytes = ggml_nbytes(tensor);
        std::vector<uint8_t> raw_data(tensor_bytes);
        ggml_backend_tensor_get(tensor, raw_data.data(), 0, tensor_bytes);
        const void * tensor_data = raw_data.data();

        // Debug: Print tensor type for first few tensors
        static int debug_count = 0;
        if (debug_count < 5) {
            LLAMA_LOG_INFO("%s: vDPE DEBUG: Tensor '%s' type=%s (id=%d), buffer=%p\n",
                          __func__, tensor_name, ggml_type_name(tensor->type), (int)tensor->type, (void*)tensor->buffer);
            debug_count++;
        }
        ...
        switch (tensor->type) {
            case GGML_TYPE_F32:
                ...
                memcpy(f32_data.data(), tensor_data, f32_size);
                data_retrieved = true;
                break;
            ...
            case GGML_TYPE_F16:
                ...
                ggml_fp16_to_fp32_row(
                    reinterpret_cast<const ggml_fp16_t*>(tensor_data),
                    f32_data.data(),
                    nelements
                );
                data_retrieved = true;
            ...
            case GGML_TYPE_Q4_1:
                dequantize_row_q4_1(
                    reinterpret_cast<const block_q4_1*>(tensor_data),
                    f32_data.data(),
                    nelements
                );
                data_retrieved = true;
            ...
            case GGML_TYPE_Q5_1:
                dequantize_row_q5_1(
                    reinterpret_cast<const block_q5_1*>(tensor_data),
                    f32_data.data(),
                    nelements
                );
                data_retrieved = true;
            ...
            case GGML_TYPE_Q8_K:
                dequantize_row_q8_K(
                    reinterpret_cast<const block_q8_K*>(tensor_data),
                    f32_data.data(),
                    nelements
                );
                data_retrieved = true;
            ...
            case GGML_TYPE_IQ4_NL:
                dequantize_row_iq4_nl(
                    reinterpret_cast<const block_iq4_nl*>(tensor_data),
                    f32_data.data(),
                    nelements
                );
                data_retrieved = true;
            ...
            default:
                LLAMA_LOG_WARN("%s: vDPE: Unsupported tensor type for '%s': %s (type_id=%d)\n",
                        __func__, tensor_name, ggml_type_name(tensor->type), (int)tensor->type);
                break;
        }

        // If dequantization failed, skip this tensor
        if (!data_retrieved) {
            LLAMA_LOG_WARN("%s: vDPE: Skipping tensor '%s' type=%s (id=%d) (couldn't dequantize to F32)\n",
                          __func__, tensor_name, ggml_type_name(tensor->type), (int)tensor->type);
            continue;
        }

        // Write F32 data to workspace
        workspace->write_direct(current_offset, f32_data.data(), f32_size);

        // Create manifest entry
        VDPETensorEntry entry;
        entry.name = std::string(tensor_name);
        ...
        if (saved_count == 0) {
            LLAMA_LOG_INFO("%s: vDPE DEBUG: First entry added - name='%s', offset=%zu, size=%zu, manifest_entries.size()=%zu\n",
                          __func__, entry.name.c_str(), entry.offset, entry.size, manifest_entries.size());
        }

        // Update offset for next tensor
        current_offset += f32_size;
        saved_count++;

        if (saved_count % 50 == 0) {
            LLAMA_LOG_INFO("%s: vDPE: Saved %zu/%zu tensors to workspace...\n", __func__, saved_count, tensors_by_name.size());
        }
    }

    // Save manifest file
    std::string manifest_path = cache_dir + "/workspace_manifest.bin";
    LLAMA_LOG_INFO("%s: vDPE DEBUG: About to save manifest with %zu entries (saved_count=%zu)\n",
                   __func__, manifest_entries.size(), saved_count);
    if (!vdpe_save_manifest(manifest_path, manifest_entries)) {
        LLAMA_LOG_ERROR("%s: vDPE: Failed to save workspace manifest\n", __func__);
        return false;
    }

    LLAMA_LOG_INFO("%s: ✅ vDPE: Successfully saved %zu/%zu F32 tensors to VPIDWorkspace (%.2f GB)\n",
                  __func__, saved_count, tensors_by_name.size(), (double)current_offset / (1024.0 * 1024.0 * 1024.0));

    return (saved_count == tensors_by_name.size());
}
...
bool llama_model::load_tensors(llama_model_loader & ml) {
    ...
    // SnapLLM vDPE: Track if tensors were loaded from cache
    bool vdpe_tensors_loaded_from_cache = false;
    bool vdpe_use_workspace = false;
    std::unordered_map<std::string, VDPETensorEntry> vdpe_manifest_map;
    snapllm::VPIDWorkspace* vdpe_workspace = nullptr;

    // SnapLLM vDPE: Check if VPIDWorkspace exists (NEW APPROACH - disk-as-RAM)
    if (params.vdpe_enable && params.vpid_workspace != nullptr && params.vdpe_cache_path != nullptr) {
        LLAMA_LOG_INFO("%s: vDPE: Checking for VPIDWorkspace at: %s\n", __func__, params.vdpe_cache_path);

        // Robust vDPE Workspace Check - check all requirements
        bool workspace_exists = vdpe_check_workspace();
        bool has_memory_mapping = false;

        // If workspace exists, verify memory mapping is available
        if (workspace_exists && params.vpid_workspace != nullptr) {
            snapllm::VPIDWorkspace* vpid = static_cast<snapllm::VPIDWorkspace*>(params.vpid_workspace);
            has_memory_mapping = vpid->has_memory_mapping();
        }

        // Can only use workspace if: exists AND external tensors enabled AND has memory mapping
        bool can_use_workspace = workspace_exists && params.use_external_tensors && has_memory_mapping;

        if (can_use_workspace) {
            LLAMA_LOG_INFO("%s: vDPE: Using disk-as-RAM fast path (memory-mapped mode)\n", __func__);
            vdpe_use_workspace = true;
            vdpe_workspace = static_cast<snapllm::VPIDWorkspace*>(params.vpid_workspace);

        } else {
            // Diagnostic: explain why workspace isn't used
            if (!workspace_exists) {
                LLAMA_LOG_INFO("%s: vDPE: No workspace - will load from GGUF and create cache\n", __func__);
    } else if (!params.use_external_tensors) {
        LLAMA_LOG_INFO("%s: vDPE: GPU mode - skipping F32 cache save\n", __func__);
                LLAMA_LOG_INFO("%s: vDPE: External tensors disabled - loading from GGUF (Direct I/O mode)\n", __func__);
            } else if (!has_memory_mapping) {
                LLAMA_LOG_INFO("%s: vDPE: No memory mapping - loading from GGUF (Direct I/O mode)\n", __func__);
            }
            vdpe_use_workspace = false;
        }
    }
    ...
    // create tensors for the weights
    {
        ...
        auto create_tensor = [&](const LLM_TN_IMPL & tn, const std::initializer_list<int64_t> & ne, int flags) -> ggml_tensor * {
            ...
            // SnapLLM vDPE: If tensors were loaded from cache, return the cached tensor instead of creating a new one
            if (vdpe_tensors_loaded_from_cache) {
                for (const auto & [name, tensor] : tensors_by_name) {
                    if (name == tn.str()) {
                        ml.n_created++;  // Increment counter for validation
                        LLAMA_LOG_DEBUG("create_tensor: using cached tensor %s\n", name.c_str());
                        return tensor;
                    }
                }
            }

            // SnapLLM vDPE: If using workspace, create tensor with external pointer (NO BUFFER ALLOCATION)
            if (vdpe_use_workspace && vdpe_workspace != nullptr) {
                std::string tensor_name = tn.str();
                auto it = vdpe_manifest_map.find(tensor_name);

                if (it != vdpe_manifest_map.end()) {
                    const VDPETensorEntry& entry = it->second;

                    // Get memory-mapped pointer from workspace
                    const float* mapped_ptr = vdpe_workspace->read_direct<float>(entry.offset, entry.size / sizeof(float));

                    if (mapped_ptr != nullptr) {
                        // Create tensor structure (lightweight, no buffer allocation)
                        ggml_tensor * tensor = ml.create_tensor(ctx, tn, ne, flags);

                        // Wire the external pointer immediately
                        // This sets tensor->data to the memory-mapped pointer without allocating buffer
                        if (llama_model_set_external_tensor(this, tensor_name.c_str(),
                                                           const_cast<float*>(mapped_ptr), GGML_TYPE_F32)) {
                            LLAMA_LOG_DEBUG("create_tensor: using workspace tensor %s (memory-mapped, zero RAM)\n", tensor_name.c_str());
                            return tensor;
                        } else {
                            LLAMA_LOG_WARN("create_tensor: failed to set external pointer for %s, falling back to normal creation\n", tensor_name.c_str());
                        }
                    } else {
                        LLAMA_LOG_WARN("create_tensor: failed to map tensor %s from workspace\n", tensor_name.c_str());
                    }
                }
            }
    ...
    // SnapLLM: Skip buffer allocation if using external tensors (vPID integration)
    ...
    if (!params.use_external_tensors && !vdpe_tensors_loaded_from_cache && !vdpe_use_workspace) {
        // Normal path: allocate backend buffers
        ...
        // SnapLLM: External tensors path - skip buffer allocation
        LLAMA_LOG_INFO("%s: skipping backend buffer allocation (use_external_tensors = true)\n", __func__);

        // Still need to populate ctxs_bufs with contexts (no buffers) so tensors_by_name can be populated
        for (auto & [buft, ctx_ptr] : ctx_map) {
            ggml_context * ctx = ctx_ptr.get();
            ...
            // Add context with empty buffer list
    ...
    // SnapLLM vDPE: Wire F32 data from VPIDWorkspace if available (disk-as-RAM fast path)
    if (vdpe_use_workspace) {
        LLAMA_LOG_INFO("%s: vDPE: Wiring F32 tensors from VPIDWorkspace...\\n", __func__);
        if (vdpe_wire_workspace()) {
            LLAMA_LOG_INFO("%s: ✅ vDPE: Successfully wired F32 tensors (disk-as-RAM)!\\n", __func__);
        } else {
            LLAMA_LOG_ERROR("%s: vDPE: Failed to wire F32 tensors from workspace\\n", __func__);
            return false;
        }
    }

    // load tensor data
    // SnapLLM: Skip data loading if using external tensors (vPID integration)
    if (!params.use_external_tensors && !vdpe_tensors_loaded_from_cache && !vdpe_use_workspace) {
        ...
        LLAMA_LOG_INFO("%s: skipping tensor data loading (use_external_tensors = true)\n", __func__);
        LLAMA_LOG_INFO("%s: external tensors must be set via llama_model_set_external_tensor()\n", __func__);
    }
    ...
    // SnapLLM vDPE: Save tensors to F32 cache if enabled and cache doesn't exist
    // Skip if already wired from workspace (vdpe_use_workspace=true means tensors are already in workspace)
    // GPU mode early exit - don't save F32 cache
        if (!params.use_external_tensors) {
        LLAMA_LOG_INFO("%s: vDPE: Skipping F32 cache save (GPU mode - external_tensors=false)\n", __func__);
    } else 
    if (params.vdpe_enable && !vdpe_tensors_loaded_from_cache && !vdpe_use_workspace && params.vdpe_cache_path != nullptr && params.use_external_tensors) {
        LLAMA_LOG_INFO("%s: vDPE: Saving F32 tensors to cache...\n", __func__);
        if (vdpe_save_cache(ml)) {
            LLAMA_LOG_INFO("%s: ✅ vDPE: F32 cache saved successfully to: %s\n", __func__, params.vdpe_cache_path);
        } else {
            LLAMA_LOG_WARN("%s: ⚠️ vDPE: Failed to save F32 cache (will retry on next load)\n", __func__);
        }
    } else if (vdpe_use_workspace) {
        LLAMA_LOG_INFO("%s: ✅ vDPE: Skipping save - tensors already in workspace (disk-as-RAM active)\n", __func__);
    } else if (!params.use_external_tensors) {
        LLAMA_LOG_INFO("%s: vDPE: GPU mode - skipping F32 cache save\n", __func__);
    }
...
llama_model_params llama_model_default_params() {
    llama_model_params result = {
        ...
        /*.no_host                     =*/ false,
        /*.use_external_tensors        =*/ false,  // SnapLLM: default to normal loading
        /*.vdpe_enable                 =*/ false,  // SnapLLM vDPE: disabled by default
        /*.vdpe_cache_path             =*/ nullptr, // SnapLLM vDPE: no cache path by default
        /*.vpid_workspace              =*/ nullptr, // SnapLLM: no VPIDWorkspace by default
    };

    return result;
}

// SnapLLM: Set external tensor data (for vPID integration)
bool llama_model_set_external_tensor(
    llama_model * model,
    const char * tensor_name,
    void * data,
    enum ggml_type type) {

    // Find tensor in model (tensors_by_name is a vector of pairs)
    ggml_tensor * tensor = nullptr;
    ...
    if (tensor == nullptr) {
        LLAMA_LOG_ERROR("%s: tensor '%s' not found in model\n", __func__, tensor_name);
        return false;
    }
    ...
    // Nullify buffer to force use of raw data pointer
    tensor->buffer = nullptr;

    LLAMA_LOG_INFO("%s: set external tensor '%s' (type=%s, size=%zu bytes)\n",
                   __func__, tensor_name, ggml_type_name(type),
                   (size_t)(tensor->nb[3] * tensor->ne[3]));
